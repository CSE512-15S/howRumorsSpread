...
var DBServer = new Server('localhost', 27017),
    ...
    timeBins = ['minute'], // Note: millisecond, second, and hour are also supported
    timestampThreshold = [0, Date.now() * 1000] // Defaults to 0 to current time
  ...
  function rawProjection () {
    ...
    var aggregationQuery = [
      {'$match' : {
        ...
        'timestamp_ms' : {
          '$gte' : timestampThreshold[0],
          '$lte' : timestampThreshold[1]
        }
      }}, projection1,
...
function spaghettiGroupings() {
  var tweets = db[collectionName].find({'retweeted_status': {$exists: false}}, { 'id': 1, 'codes': 1, 'text': 1, 'created_ts': 1, 'favorite_count': 1, 'user.id': 1, 'user.name': 1, 'user.name': 1, 'user.screen_name': 1, 'user.followers_count': 1, 'user.profile_image_url': 1, 'user.verified': 1, 'user.favorite_count': 1 }).toArray();
  var retweets = db[collectionName].find({retweeted_status: {$exists: true}}, { 'created_ts': 1, 'user.followers_count': 1, 'user.verified': 1, 'user.screen_name': 1, 'retweeted_status.id': 1 }).toArray();
